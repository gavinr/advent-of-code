...
const intCode = async (inn, a) => {
  ...
  let found = false;
  let curNodes = [tree]; // our "queue" for the depth-first-search
  let depth = 0;
  let oxygenSourceTreeNode;
  while(found == false) {
    depth = depth + 1;
    ...
    if(foundItems) {
      found = true;
      oxygenSourceTreeNode = foundItems;
    } else {
  ...
  // make oxygenSourceTreeNode the root of the tree:
  let lastNode = undefined;
  let currentNode = oxygenSourceTreeNode;
  tree = currentNode;
  foundTop = false;
  while(foundTop === false) {
    if(currentNode.parent) {
      const tempHoldToParent = currentNode.parent;
      currentNode.children.push(currentNode.parent);
      currentNode.parent = lastNode;
      
      const index = currentNode.children.indexOf(lastNode);
      if(index > -1) {
        currentNode.children.splice(index, 1);
      }
      lastNode = currentNode;
      currentNode = tempHoldToParent;
    } else {
      foundTop = true;
  ...
  console.log('done re-aligning tree', tree);



  // do a depth first search again, this time just watiting until the very bottom (no children left)
  found = false;
  curNodes = [tree]; // our "queue" for the depth-first-search
  depth = 0;
  while(found == false) {
    depth = depth + 1;
    // queue the children of all current nodes
...
console.log("RESULT: ", intCode(input)); // answer: 320