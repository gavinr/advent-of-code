...
const intcodeRun = (inn, a) => {
  ...
  var count = 0;
  while (input[currentLocation] !== 99 && count < 20000000) {
    ...
    if (opCode === 1) {
      ...
      console.log(asciiArrayToString(fullOutput[fullOutput.length-1]));
      fullOutput.push([]);
      // input
      const a1 = a.shift();
      console.log('a1:', a1);
      ...
      // read in last output:
      // if(fullOutput[fullOutput.length-1].length > 2256) {
        
      // }
      

      fullOutput[fullOutput.length-1].push(getParam(1));
      // return getParam(1);
      currentLocation = currentLocation + 2;
    } else if (opCode === 5) {
      // jump-if-true
...
const toGrid = (input) => {
  ...
  for(let i = 0; i < input.length; i++) {
    ...
    if(val === 46) {
      ...
      console.error('Error - found letter we do not know!', val);
    }
  }
}
...
const runProgram = (inn) => {
  ...
  // }, 0);
  const mvmtString = toAscii('B,C,C,A,C,A,C,A,B,B');
  console.log('mvmtString:', mvmtString);
  const mvmtFunctionA = toAscii('L,12,L,6,L,4,L,4');
  console.log('mvmtFunctionA:', mvmtFunctionA);

  const mvmtFunctionB = toAscii('L,6,R,8,L,4,R,8,L,12');
  console.log('mvmtFunctionB:', mvmtFunctionB);

  const mvmtFunctionC = toAscii('L,12,R,10,L,4');
  console.log('mvmtFunctionC:', mvmtFunctionC);

  // only works in "live" mode?
  // https://www.reddit.com/r/adventofcode/comments/ec5dst/2019_day_17_part_2_no_output_from_the_vac_bot/
  const result = intcodeRun(inn, [... mvmtString, 10, ... mvmtFunctionA, 10, ... mvmtFunctionB, 10, ... mvmtFunctionC, 10, 89, 10]); // 110
  return result;
}
