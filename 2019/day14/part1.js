...
// prettier-ignore
const input = '2 VPVL, 7 FWMGM, 2 CXFTF, 11 MNCFX => 1 STKFG;17 NVRVD, 3 JNWZP => 8 VPVL;53 STKFG, 6 MNCFX, 46 VJHF, 81 HVMC, 68 CXFTF, 25 GNMV => 1 FUEL;22 VJHF, 37MNCFX => 5 FWMGM;139 ORE => 4 NVRVD;144 ORE => 7 JNWZP;5 MNCFX, 7 RFSQX, 2 FWMGM, 2 VPVL, 19 CXFTF => 3 HVMC;5 VJHF, 7 MNCFX, 9 VPVL, 37 CXFTF => 6GNMV;145 ORE => 6 MNCFX;1 NVRVD => 8 CXFTF;1 VJHF, 6 MNCFX => 4 RFSQX;176 ORE => 6 VJHF'; // 180697 
...
const retFind = (qty, element, inputObject, elementInterestedIn) => {
  // console.log('retFind', qty, element, oreOutputs);
  if(element == elementInterestedIn) {
    return qty;
  } else {
    // recurse
    ...
    return inputs.reduce((acc, cur) => {
      console.log('looking for ', cur[0], ' ', cur[1], ' -- multiplier ', multiplier);
      return acc + (multiplier * retFind(cur[0], cur[1], inputObject, elementInterestedIn));
    }, 0);

    // // console.log('inputs for ', element, ' ARE: ', inputs);
    // return inputs.reduce((input) => {
    //   return retFind(input[0], input[1], inputObject, oreOutputs);
    // });
  }
}
...
const runProgram = (input) => {
  ...
  // our recursive function cannot go all the way down to ORE - we need 
  // to find which inputs are produced by ORE and only have our recursive 
  // function only go down to that level, so that we can aggregate when
  // finding the total ore
  const oreOutputs = getOreOutputs(inputObject);
  console.log('oreOutputs:', oreOutputs);
  console.log('oreOutputs:', Object.keys(oreOutputs));

  const totalNeedPerOreOutput = {}
  Object.keys(oreOutputs).forEach((oreOutputElement) => {
    totalNeedPerOreOutput[oreOutputElement] = retFind(1, 'FUEL', inputObject, oreOutputElement);
  });
  console.log('totalNeedPerOreOutput:', totalNeedPerOreOutput);

  let totalNeeded = 0;
  Object.keys(totalNeedPerOreOutput).forEach((element) => {
    const totalTimes = Math.ceil(totalNeedPerOreOutput[element]/oreOutputs[element].amountOutput);
    const oreInputNeeded = oreOutputs[element].amountOreInput;
    
    const additional = totalTimes * oreInputNeeded;
    console.log(element, ': ' , totalTimes, ' needed, times ', oreInputNeeded, '(ore) = ', additional);
    totalNeeded = totalNeeded + additional;
  });

  // const neededOreOutputs = retFind(1, 'FUEL', inputObject, oreOutputs.map((arr) => {
  //   return arr[1];
  // }));
  // console.log('neededOreOutputs', neededOreOutputs);
